/**
 * train.c - Programa principal de entrenamiento MLP (versión OpenMP)
 *
...
{
    printf("=================================================================\n");
    printf("MLP MNIST - Implementación OpenMP en C\n");
    printf("=================================================================\n\n");
    ...
    if (!mlp)
    {
        fprintf(stderr, "Error creando MLP\n");
        free_dataset(train);
        free_dataset(test);
        return 1;
    }

    // Obtener número de threads de OpenMP
    int num_threads = 1;
#pragma omp parallel
    {
#pragma omp master
    ...
    printf("OpenMP habilitado: %d threads\n", num_threads);
    ...
    FILE *csv = fopen("../../results/raw/c_openmp.csv", "w");
    if (csv)
    {
        ...
        fprintf(csv, "c_openmp,c,openmp,%d,%d,%d,%.3f,%d,%.2f,%.2f,%.4f,%.4f,TBD,openmp_parallel\n",
                num_threads, BATCH_SIZE, EPOCHS, LEARNING_RATE, HIDDEN_NEURONS,
                total_time, total_time / EPOCHS, test_loss, test_accuracy);
        fclose(csv);
        printf("Resultados guardados en: results/raw/c_openmp.csv\n");
    }
